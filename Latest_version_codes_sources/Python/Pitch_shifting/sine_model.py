...
#   Phase increment vector
    delta_phi = 2.0*(alpha-1)*np.pi*np.arange(mid_fft_a+1)*H/float(N_fft_a)
...
#        argX_u=np.unwrap(arg_X)
#        arg1_plot.append(argX_u[peak_loc])      
...
#       Use the closest arg to peak_arg as peak_arg (avoid interpolation problem caused by unwrap) 
        delta_phase_index = np.arange(np.size(delta_phi))
        delta_phase_value = np.unwrap((delta_phi)*float(k))
        delta_phase = interpolate.interp1d(delta_phase_index, delta_phase_value,kind='nearest')

        peak_phase = np.interp(peak_loc_interp,np.arange(np.size(arg_X)),np.unwrap(arg_X))+delta_phase(peak_loc)
...
#       Synthesize signal and remove zero-phase correction
        Y = sine_synthesis(peak_loc_interp*alpha*N_fft_s/float(N_fft_a),peak_mag,peak_phase,N_fft_s,N_fft_s)
...
# -----------------------------------------------------------------------------
#    print delta_phi[peak_loc]
#    print np.max(argX_shift)
