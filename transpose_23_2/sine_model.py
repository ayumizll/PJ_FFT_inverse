...
#   Debug variable    
    k = 0
    lobe_phase = np.zeros(10000)
    lobe_phase_2 = np.zeros(10000)    
    
#   ---------------------------------------------------------------------------
#   Frequency shift factor
#    alpha = 1.5
#   Phase increment vector
    delta_phi = 2.0*alpha*np.pi*np.arange(mid_fft_a+1)*H/N_fft_a
...
#       Interpolated values
        ...
        if (k == 0):
            arg_X_shifted = np.unwrap(arg_X*10.0)/10.
...
#
#        lobe_phase[k] = peak_phase
#        lobe_phase_2[k] = arg_X_shifted[peak_loc]
#
#        print[peak_loc]
